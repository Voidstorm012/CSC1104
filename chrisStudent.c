...
// Define waveform configurations (MODDED)
typedef struct {
    int frequency;
    int dutyCycle;
} WaveformConfig;

// Define waveform data buffer size (MODDED)
#define DATA_BUFFER_SIZE 3000 // enough for 60 seconds at 50 data points per second

int redData[DATA_BUFFER_SIZE];
int greenData[DATA_BUFFER_SIZE];
int dataIndex = 0;

WaveformConfig redConfig, greenConfig;
...
//MODDED CODE
void blinkLED(int pin, WaveformConfig config) {
    int cycleTime = 1000 / config.frequency;
    int onTime = (cycleTime * config.dutyCycle) / 100;
...
void saveToFile() {
    FILE *file = fopen("waveforms.csv", "w");
    if (!file) {
        printf("Error opening file!\n");
        return;
    }

    // Write waveform configurations
    fprintf(file, "Red LED Frequency: %d Hz, Duty Cycle: %d%%\n", redConfig.frequency, redConfig.dutyCycle);
    fprintf(file, "Green LED Frequency: %d Hz, Duty Cycle: %d%%\n\n", greenConfig.frequency, greenConfig.dutyCycle);

    // Write waveform data
    for (int i = 0; i < dataIndex; i++) {
        fprintf(file, "%d,%d\n", redData[i], greenData[i]);
    }
