...
void blinkLedWithConfig(int blinkLed, int blinkFrequency, int blinkBrightness)
{
    ...
    for (int blink = 0; blink < 20;)
    {
        ...
        while (blink < 20)                                                  // Timer
        {
            ...
            if (currentMillis - previousMillis >= onOffTime)                // Tells the LED when to turn on or off (from the user input frequency)
            {
                ...
                {
                    ledState = HIGH;
                    printf("Set to High");                                  // Just for testing to show LED being set to HIGH (on)
                    //softPwmWrite(blinkLed, blinkBrightness);              // DO NOT REMOVE (Commented out to test with printf)
                }
                else
                {
                    ledState = LOW;
                    printf("Set to Low");                                   // Just for testing to show LED being set to LOW (off)
            ...
            dataArray[rows - 1][1] = currentTime;
            rows++;
    ...
    // Save to CSV
    FILE *saveFile = fopen("waveformData.csv", "a"); // Open file in append mode
    ...
    free(dataArray);                                                        //  Free Memory (Part b)
}
